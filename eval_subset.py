...
if __name__ == '__main__':
    ...
    parser.add_argument('--subset', default=0.1, type=float, help='Subset ratio')
    ...
    model_path = f'main_result/{args.dataset}{args.subset}/results_{args.algo}_batch{args.batch_size}/{model_name}_{args.checkpoint}.pth'
    result_path = f'main_result/{args.dataset}{args.subset}/results_{args.algo}_batch{args.batch_size}/linear_{model_name}_statistics_{args.checkpoint}.csv'
    ...
    start_class = -1 if args.dataset == 'stl10' else 0
    class_idx = np.array([np.where(class_idx == i)[0] for i in range(start_class, num_class)])

    indices = []
    for i in range(num_class):
        idx = class_idx[i]
        np.random.shuffle(idx)
        indices.extend(idx[:int(len(idx) * args.subset)])
